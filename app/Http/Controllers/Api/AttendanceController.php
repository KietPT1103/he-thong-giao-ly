...
class AttendanceController extends ApiController {
 ...
 public function store(Request $request,CatechismClass $class){$this->authorize('takeAttendance',$class);$data=$request->validate(['held_at'=>'required|date','note'=>'nullable|string']);if($class->attendanceSessions()->where('held_at',$data['held_at'])->exists())return response()->json(['success'=>false,'message'=>'Attendance session already exists.'],422);$session=$class->attendanceSessions()->create($data+['taken_by'=>$request->user()->id]);return $this->success($session,'Attendance session created.');}
