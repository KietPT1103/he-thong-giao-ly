...
class UpsertAssignmentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'targets' => $draft
                ? ['sometimes', 'array', 'max:30']
                : ['required', 'array', 'min:1', 'max:30'],
            ...
            'questions.*.prompt' => [$draft ? 'nullable' : 'required', 'string', 'max:5000'],
            ...
            'questions.*.options.*.content' => [$draft ? 'nullable' : 'required_with:questions.*.options', 'string', 'max:1000'],
            'questions.*.options.*.is_correct' => ['required_with:questions.*.options', 'boolean'],
            'questions.*.accepted_answers' => ['nullable', 'array', 'max:20'],
            'questions.*.accepted_answers.*' => ['nullable', 'string', 'max:500'],
            'questions.*.rubric' => ['nullable', 'array', 'max:20'],
            'questions.*.rubric.*.label' => [$draft ? 'nullable' : 'required_with:questions.*.rubric', 'string', 'max:200'],
