...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $parish = Parish::create(['name' => 'Giáo xứ An Bình', 'code' => 'AN-BINH']);
        ...
        foreach (range(1, 8) as $i) {
            $u = User::create(['name' => "Giáo lý viên $i", 'email' => $i === 1 ? 'teacher@giaoly.test' : "teacher$i@giaoly.test", 'password' => $password]);
            $u->assignRole('teacher');
            $teachers->push(TeacherProfile::create(['user_id' => $u->id, 'parish_id' => $parish->id, 'code' => 'GLV-' . $i]));
        }
        $classes = collect();
        foreach (range(1, 6) as $i) {
            ...
            $class->teachers()->attach($teachers[($i - 1) % 8]->id, ['role' => 'primary']);
            ClassSchedule::create(['catechism_class_id' => $class->id, 'weekday' => 0, 'starts_at' => '08:00', 'ends_at' => '09:30']);
            $classes->push($class);
        }
        $parents = collect();
        foreach (range(1, 20) as $i) {
            $u = User::create(['name' => "Phụ huynh $i", 'email' => $i === 1 ? 'parent@giaoly.test' : "parent$i@giaoly.test", 'password' => $password]);
            $u->assignRole('parent');
            $parents->push(ParentProfile::create(['user_id' => $u->id, 'parish_id' => $parish->id, 'phone' => '090000' . str_pad((string)$i, 4, '0', STR_PAD_LEFT)]));
        }
        foreach (range(1, 30) as $i) {
            ...
            $child->parents()->attach($parents[($i - 1) % 20]->id);
