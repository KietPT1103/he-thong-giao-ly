...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $expandedTestDataset = app()->environment('testing');
        $classes = collect();
        foreach (range(1, $expandedTestDataset ? 6 : 1) as $i) {
        ...
        foreach (range(1, $expandedTestDataset ? 30 : 1) as $i) {
            $u = null;
            if ($i === 1) {
        ...
        foreach ($classes as $class) {
            ...
            foreach ($class->children as $child) {
                Attendance::create(['attendance_session_id' => $session->id, 'child_id' => $child->id, 'status' => AttendanceStatus::Present->value]);
            }
        }
    }
}
