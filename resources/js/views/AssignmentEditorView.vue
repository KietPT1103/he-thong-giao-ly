...
function apiMessage(value: unknown, fallback: string) {
    return (value as { response?: { data?: { message?: string; errors?: Record<string, string[]> } } }).response?.data?.message ?? fallback;
}
...
function payload(): AssignmentInput {
    return {
...
<template>
    <section class="editor-page">
        ...
        <nav class="stepper" aria-label="Các bước tạo bài tập">
            <button v-for="(item,index) in steps" :key="item.title" type="button" :class="{ active:step===index+1, done:step>index+1 }" :aria-current="step===index+1?'step':undefined" @click="step=index+1">
