...
import {
    ...
    CircleAlert, ClipboardCheck, Download, FilePenLine, Plus, RefreshCw, Search,
    Send, UsersRound,
...
async function saveManagement() {
    ...
    try {
        if (managementMode.value === "due") {
            if (!managementDue.value) { toast.error("Hãy chọn hạn nộp mới."); return; }
            await changeAssignmentDueDate(selected.value.id, managementDue.value);
            toast.success("Đã đổi hạn và thông báo đến Thiếu nhi.");
        } else if (managementMode.value === "accommodation") {
            if (!managementChildId.value || !managementReason.value.trim()) { toast.error("Hãy chọn Thiếu nhi và nhập lý do."); return; }
            await setAssignmentAccommodation(selected.value.id, managementChildId.value, { due_at: managementDue.value || null, extra_attempts: managementExtraAttempts.value, reason: managementReason.value });
            toast.success("Đã cấp ngoại lệ và thông báo cho Thiếu nhi.");
        } else {
            if (!managementReason.value.trim()) { toast.error("Hãy nhập lý do thu hồi."); return; }
    ...
    <section class="assignment-page">
        ...
        <section class="assignment-register" aria-labelledby="assignment-register-title">
            ...
            <div class="filters">
                ...
                <ASelect v-model:value="status" allow-clear placeholder="Tất cả trạng thái" :options="statusOptions" aria-label="Lọc theo trạng thái" />
                <ASelect v-model:value="classId" allow-clear placeholder="Tất cả lớp" :options="classes.map(item=>({value:item.id,label:`${item.name} · ${item.code}`}))" aria-label="Lọc theo lớp" />
        ...
        <AModal :open="managementOpen" centered :title="managementMode==='due'?'Đổi hạn nộp':managementMode==='accommodation'?'Ngoại lệ cho Thiếu nhi':'Thu hồi bài tập'" :footer="null" width="520px" @cancel="managementOpen=false">
            <div class="management-form">
                ...
                <label v-if="managementMode==='accommodation'"><span>Thiếu nhi *</span><ASelect v-model:value="managementChildId" show-search :options="recipientOptions" placeholder="Chọn người nhận" /></label>
                <label v-if="managementMode!=='withdraw'"><span>{{ managementMode==='due' ? 'Hạn nộp mới *' : 'Gia hạn đến' }}</span><input v-model="managementDue" type="datetime-local"></label>
...
@media(max-width:640px){.assignment-page{gap:14px}.queue-items{grid-template-columns:1fr}.queue-items button+button{border-top:1px solid #e7edf5;border-left:0}.filters{grid-template-columns:1fr;padding:12px}.filters>:first-child{grid-column:auto}.assignment-row{grid-template-columns:minmax(0,1fr) auto;gap:10px;padding:14px}.row-meta{grid-column:1}.row-count{grid-column:2;grid-row:1/3}.assignment-detail{padding:18px 14px}.detail-actions{grid-template-columns:1fr}.state-panel{align-items:flex-start;flex-direction:column}.state-panel .ant-btn{margin-left:0}}
